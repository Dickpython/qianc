...
def parse_normal_time(vals, default, missing_value=[],**kwargs):
...
def parse_24month(vals, default, param, missing_value=[], **kwargs):
    ...
    def _LPS(x):
        if x != "" and x is not None and x not in missing_value:
            ...
            return [default]*len(param)
    if vals.shape[0] != 0 :
        return np.array([_LPS(v[0]) for v in vals]) 
    else:
        return np.array([[default]*len(param)])


def parse_merge(vals, default, missing_value=[], **kwargs):
    out = []
    for v in vals:
        if len(v) > 0:
